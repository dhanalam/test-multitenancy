        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('tenant.tasks.store', ['tenant' => tenant('id')]) }}">
                        @csrf
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <a href="{{ route('tenant.tasks.index', ['tenant' => tenant('id'), request()->has('project_id') ? 'project_id=' . request()->project_id : '']) }}" class="px-4 py-2 bg-gray-300 text-gray-800 rounded hover:bg-gray-400 mr-2">
                                {{ __('Cancel') }}
                            </a>
                            <x-primary-button>
                                {{ __('Create') }}
